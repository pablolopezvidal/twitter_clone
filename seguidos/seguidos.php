...
$sql ="SELECT * FROM social_network.publications join users on publications.userId = users.id where userId in(SELECT userToFollowId FROM social_network.follows where users_id=$idUsuario)";
...
<body>
    ...
    <div id="contenido">
      <?php while ($row = mysqli_fetch_array($query)): ?>
              ...
              <div class="caja">
                  <a href="/pagina_mundo/perfilUsuario.php?username=<?=$row['username'] ?>"><?= $row['username'] ?></a> <!--que esto sea un <a> con un metodos post para poder almacenar el nombre de usuario y arrais de ahi imprimir todas las cosas de ese susuario -->
                  <h3><?= $row['text']?></h3>
